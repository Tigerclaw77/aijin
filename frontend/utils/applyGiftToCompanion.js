// utils/applyGiftToCompanion.js

import { supabase } from "./supabaseClient";
import { giftCatalog } from "./giftUtils";
 ...
 */
export async function applyGiftToCompanion({ user_id, companion_id, giftName }) {
  ...
  const verbalXP = (effect.type === "verbal" || effect.type === "both") ? baseXP * decayFactor : 0;
  const physicalXP = (effect.type === "physical" || effect.type === "both") ? baseXP * decayFactor : 0;
  ...
  if (companion.soft_deleted || companion.is_paused) {
    return { success: false, error: "Cannot apply gift to paused or deleted companion." };
  }
